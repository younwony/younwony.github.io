...
**2. 멀티소스 API 파이프라인**
- **선택**: TikTok API + Ensemble API + EchoTik API 조합
- **Why**: TikTok API로 팔로워/좋아요 메인 데이터, Ensemble/EchoTik으로 이메일·US 거주지 세부 데이터 수집
- **Why Not 단일 소스**: 각 API의 데이터 커버리지 상이. 멀티소스 조합으로 데이터 풍부성 확보
...
> MD팀 협업 기반 전략 상품 타겟팅으로 매출 10% 상승

- 카테고리/브랜드 타겟팅 기반 네이버 최저가 자동 조정 프로세스 구축
- MD팀 선별 상품 집중 모니터링 + 자동화 하이브리드 구조 정착
- 최저가 미싱 분석 → 마진율 계산 → 가격 조정 협업 체계 확립
