...
**[프로젝트 배경]**
마케팅팀이 틱톡/인스타그램을 직접 탐색하고 엑셀에 수기 입력하여 인플루언서를 관리하고 있었습니다. 하루 수집 가능한 인플루언서가 수십 명 수준에 불과해 대형 캠페인 진행이 불가능했습니다. TikTok, Instagram 등 각 플랫폼별로 팔로워, 좋아요, 이메일 등 지표가 분산되어 있어 인플루언서 성과를 통합 비교할 기준이 없었고, 200만 건 이상 데이터에서 복합 조건 검색 시 RDB 쿼리 타임아웃(10초 이상)이 발생하는 상황이었습니다. CDN 링크 만료로 인한 프로필 이미지 유실도 다수 발생했습니다.
...
**Tech Stack**: Elastic APM, TDD (JUnit, Mockito), GitHub PR Review, Slack API, Shell Script
...
### Seller 어드민 TDD 도입 및 핵심 기능 테스트 체계 구축
...
- **조회 성능 80% 개선 (5초 → 1초 미만)**: 테스트 과정에서 발견된 Slow Query에 대해 실행 계획 분석 및 인덱스 튜닝. N+1 문제 해결을 위한 Fetch Join 적용.
