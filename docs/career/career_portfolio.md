...
**역할**: 백엔드 아키텍처 설계 및 구현 주도 | PM·마케팅팀과 요구사항 정의 및 검증 협업
...
**[주요 성과]**
- **검색 성능 10배 향상 (10초+ → 1초 이내)**: Elasticsearch 역색인 기반 검색 엔진을 구축하여 200만 건 복합 조건 검색 시 RDB 타임아웃 문제 해결. 동적 필터 100가지 조합을 커버하기 위해 RDB 복합 인덱스 대신 역색인 방식 선택.
...
### 네이버 쇼핑 Dynamic Pricing 시스템 (2023.01 - 현재)
...
**[주요 성과]**
- **대상 상품 매출/트래픽 10% 상승**: 카테고리/브랜드 타겟팅 기반 네이버 최저가 자동 조정 프로세스 구축으로 가격 경쟁력 확보.
- **MD팀 협업 체계 구축**: MD팀이 선별한 모델/브랜드 기준 집중 모니터링 체계 구축. 가격 변동 데이터를 MD팀과 공유하며 최저가 조정 방향 협의.
- **최저가 미싱 분석 체계 확립** (진행 중): 네이버 최저가 API로 미싱 데이터 비교 분석 → 마진율 계산 → 가격 조정안 공유 → MD팀 매뉴얼 반영 핑퐁 체계 확립. 할인율 계산 기반 매출 증대 방향 제시.
...
**역할**: 해시 기반 검증 시스템 설계 및 구현 주도 | CS팀과 클레임 패턴 분석 협업
...
**역할**: AI 도구 도입 및 활용 프로세스 구축 주도 | 팀 내 AI 활용 문화 전파
