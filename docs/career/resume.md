...
> 200만 인플루언서 데이터 자동 수집으로 마케팅 커버리지 20배 확대, 검색 100배 개선

- **문제**: 수작업 수집으로 일 수십 명 한계, RDB 검색 10초+ 타임아웃
- **해결**: Elasticsearch 검색 엔진 + 멀티소스(TikTok/Ensemble API) 수집 파이프라인 설계
- **성과**: 데이터 풀 10만→200만(20배↑), 검색 10초→0.1초(100배↑), 리포팅 **Zero Ops**
...
- **해결**: ETag + MD5 해시 이중 검증, HEAD 요청 우선으로 네트워크 비용 80% 절감
- **성과**: 이미지 불일치 클레임 **Zero**, 반품 비용 월 100만원+ 절감, 검수 **Zero Ops**
- **기술**: Spring Boot, AWS S3, Image Hashing (MD5/ETag)

---

#### ChatOps 기반 운영 프로세스 자동화

> 운영 요청 채널 Slack 통합으로 수동 업무 40% → 0% 완전 자동화

- **문제**: 발주/클레임 요청이 이메일/메신저/구두로 분산, 운영팀 40% 시간 단순 반복 업무
- **해결**: Slack 기반 ChatOps 구축, 버튼 클릭만으로 DB 처리 완료
...
- **Claude Code 활용**: 운영 도구 개발 시간 90% 단축 (1-2일 → 1-2시간)
...
> 판매자 정산/주문 조회 쿼리 최적화로 응답 시간 80% 단축 (5초 → 1초 미만)

- 실행 계획 분석 통한 인덱스 튜닝 및 불필요한 JOIN 제거
- **기술**: Java, Spring Framework, Oracle
...
> Elasticsearch 기반 검색 엔진 구축으로 라이선스 비용 절감, 데이터 등록 70% 단축

- **문제**: 상용 솔루션 연간 수천만원 비용, 단건 Insert로 일 8시간+ 소요
- **해결**: ES + Nori 한글 형태소 분석기, Bulk Insert 전환
- **성과**: 라이선스 비용 **100% 절감**, 등록 속도 **70% 단축**, **2021년 우수사원** 선정
- **기술**: Java, Spring Framework, Elasticsearch, Nori Analyzer, Oracle
