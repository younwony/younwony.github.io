...
200만 건 규모의 대용량 데이터 파이프라인 설계부터 ChatOps 기반 운영 자동화까지, 시스템 전 영역에서 성능 최적화와 운영 효율화를 동시에 달성해왔습니다. 최근에는 AI 에이전트(Claude Code)와 MCP를 활용하여 개발 생산성을 혁신하고, 팀 전체의 역량 향상을 이끌고 있습니다.

**핵심 역량:**
- **대용량 시스템 설계**: Elasticsearch 기반 200만 건 실시간 검색, 병렬/비동기 처리 아키텍처 설계
- **자동화 & Zero Ops**: ChatOps, 리포팅 자동화, API 문서 자동화로 운영 업무 완전 자동화
- **AI 기반 생산성 혁신**: Claude Code + MCP 활용으로 개발/문서화 업무 90% 시간 단축
- **Cross-functional 협업**: PM/마케팅/MD/CS팀과 직접 소통하며 기술 제약을 비즈니스 솔루션으로 전환
...
- **AI 활용 생산성 혁신**: Claude Code + MCP 도입으로 운영 도구 개발 및 API 문서화 업무 90% 자동화
...
- **검색 성능 최적화**: Elasticsearch 도입으로 복합 조건 검색 성능 100배 개선 (10초 → 0.1초)
...
**사용 기술:**
- Java, Spring Boot, Elastic APM, TDD (JUnit, Mockito), Slack API, Shell Script, Claude Code (AI Agent), MCP (Atlassian)
...
#### 프로젝트: Seller 어드민 TDD 도입 및 핵심 기능 테스트 체계 구축
...
- **쿼리 최적화**: 테스트 과정에서 발견된 Slow Query에 대해 실행 계획 분석 및 인덱스 튜닝
