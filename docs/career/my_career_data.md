...
- **멀티소스 데이터 파이프라인 설계**: Instagram/YouTube/TikTok 등 다중 플랫폼 데이터 통합 수집 아키텍처 설계. TikTok API, Ensemble API, EchoTik API 및 Web Scraper를 조합하여 병렬 처리 구조로 일 평균 5,000명 이상 자동 수집 달성
...
- **운영 한계**: 기존 자동화 시스템은 카테고리/브랜드 기준 일괄 처리만 가능, MD팀이 선별한 전략 상품에 대한 정밀 대응 불가
- **데이터 사각지대**: 최저가 미싱(누락) 데이터 발생 시 대응 체계 부재, 매출 기회 손실

**🎯 Task (과제/목표):**
- 카테고리/브랜드 기반 타겟팅으로 네이버 최저가 자동 조정 프로세스 구현
- MD팀 선별 전략 상품에 대한 집중 모니터링 및 가격 조정 협업 체계 구축
- 최저가 미싱 데이터 분석 → 마진율 계산 → 가격 조정 의사결정 지원 프로세스 확립

**⚡ Action (해결 방법):**
- **자동 프로세스 구축**: 전체 상품 대상 동적 가격 모니터링 시스템 구축. 특정 카테고리/브랜드 우선 설정, 모델번호 매핑 기반 경쟁사 최저가 수집 및 할인율 자동 적용
- **MD팀 협업 프로세스 (핑퐁 방식)**: MD팀이 타겟팅한 전략 상품 데이터 제공 → MD팀 마진율 계산 및 적정 판매가 산정 → 개발팀 할인 적용 → 네이버 최저가 달성. 핑퐁 형태의 협업으로 정밀한 가격 전략 실행
- **최저가 미싱 분석 체계**: 네이버 최저가 API로 미싱 데이터 비교 분석 → 마진율 기반 가격 조정안 공유 → MD팀 반영 협의 체계 확립
- **하이브리드 운영 구조 설계**: 자동 처리 + MD팀 전략적 수동 개입 통합. Admin 뷰 제공으로 예외 케이스 집중 관리

**✅ Result (성과):**
- **매출/트래픽 10% 동반 상승**: 가격 경쟁력 확보로 해당 상품군 매출·트래픽 각각 10% 증가
- **운영 효율 혁신**: 자동화 + MD팀 전략적 개입 하이브리드 구조 정착
- **데이터 기반 협업 체계 확립**: 최저가 분석 데이터 공유로 MD팀 의사결정 속도 향상

**💡 Impact (비즈니스 효과):**
- 가격 경쟁력 확보로 네이버 쇼핑 노출 우위 달성
- MD팀 전략 상품 집중 관리로 매출 극대화 기여
...
#### 프로젝트 7: 품절률 감소 및 데이터 분석

**📌 Situation (배경 및 문제점):**
- **재구매 유저 이탈 분석 필요**: 재구매 과정에서 이탈률이 높은 경험 지점 파악 필요
- **핵심 발견**: GA4 + BigQuery 분석 결과, 품절 경험 유저의 재구매 전환이 거의 발생하지 않음
- **취소 사유 분석**: 전체 취소 중 품절 취소 사유가 40% 차지
- **셀러 관리 한계**: 배송/주문 진행 지연 셀러 식별 및 관리 체계 부재

**🎯 Task (과제/목표):**
- 재구매 유저 이탈 원인(품절) 분석 및 해결
- 품절 유발 셀러 식별 및 패널티 정책 수립
- 품절 취소 사유 비율 감소 및 고객 경험 개선

**⚡ Action (해결 방법):**
- **재구매 이탈 분석**: GA4 + BigQuery 연동으로 유저 행동 패턴 분석 → 품절 경험 유저 재구매 전환율 급감 확인
- **품절 원인 분석**: DB 기반 상품/주문/배송/클레임 연관관계 분석. 해외 부티크 데이터 싱크(구조적 한계), 배송 2주 이상 지연 셀러(대응 가능), 주문 진행 2주 이상 지연 셀러(대응 가능) 분류
- **셀러 패널티 정책 수립**: 지연 셀러 대상 패널티 부여, 누적 시 계약 해지까지 진행
- **고객 경험 보상**: 품절 경험 고객 대상 포인트 지급으로 이탈 방지 및 재구매 유도

**✅ Result (성과):**
- **품절 취소 사유 비율 50% 감소**: 40% → 20%
- 재구매 전환율 개선
- 데이터 기반 셀러 관리 체계 구축

**💡 Impact (비즈니스 효과):**
- 고객 경험 개선으로 재구매 전환율 향상
- 셀러 품질 관리 체계 구축
- 고객 보상 정책으로 브랜드 신뢰도 향상

**🔧 Tech Decision:**
> "BigQuery + GA4 연동 - 유저 행동 데이터와 내부 DB 조인으로 재구매 이탈 원인 정확히 파악"
> "정책 + 보상 병행 - 원인 제거(셀러 패널티) + 경험 보상(포인트)으로 종합적 접근"
> "고객 관점 출발 - 단순 품절 관리가 아닌 '재구매 이탈' 고객 경험 문제로 정의"

**사용 기술:**
- BigQuery, Google Analytics (GA4), MySQL
